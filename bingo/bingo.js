  ...
  (this.col1 = []),
    ...
    (this.populateColumns = function () {
      ...
      while (this.col1.length < 5) {
        const num = Math.ceil(Math.random() * 60);
      ...
      while (this.col3.length < 5) {
        num = Math.ceil(Math.random() * 60);
        if (
          !this.col3.includes(num) &&
          !this.col1.includes(num) &&
          !this.col2.includes(num)
        ) {
          this.col3.push(num);
        }
      }
...
const changeNumForX = (numbersArray) => {
...
const lineComplete = (numbersArray) => {
...
const main = () => {
  ...
  do {
    ...
    if (
      bingo(playerCard.col1) &&
      bingo(playerCard.col2) &&
      bingo(playerCard.col3)
    ) {
      alert("¡¡¡ ~~~ BINGOOOO ~~~ !!!");
      alert(
        `Remember, the more rounds the worse the result.\n\n${playerName}, you BINGOED after ${gameRounds} rounds.`
      );

      ranking.push({ name: playerName, roundsToBingo: gameRounds });
      const sortedRanking = ranking.sort(
        (a, b) => a.roundsToBingo - b.roundsToBingo
      );

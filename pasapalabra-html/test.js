...
function showQuestions() {
    ...
    switch(true) {
        ...
        // After all question in main block are exausted, enter questions passed
        case firstPlayer.i >= questions.length && secondPlayer.i >= questions.length && firstPlayer.qPassed.length > 0 || secondPlayer.qPassed.length > 0:
            ...
            console.log('index main questions superado');
            ...
            // Show player, letter, question
            playerBox.innerText = `Jugador ${playerObj.playerName}`;
            letterBox.innerText = `Letra: \n${questions[playerObj.iQPassed].letter.toUpperCase()}`
            questionBox.innerText = `${playerObj.qPassed[playerObj.iQPassed].question[playerObj.qPassed[playerObj.iQPassed].status]}.\n\nRespuesta: `;

            // Reset answer box and maintain cursor inside it
            answerBox.value = '';
            answerBox.focus();
            console.log(playerObj.playerName, playerObj.iQPassed);
            break;

        // No more questions on main block and no questions passed
        case firstPlayer.i >= questions.length && secondPlayer.i >= questions.length && firstPlayer.qPassed.length === 0 && secondPlayer.qPassed.length === 0:
            ranking(firstPlayer, secondPlayer);
...
function showPassedQuestions() {

    console.log('inside showPassedQuestions');
...
// This function will loop on the passed questions array until it ends it for both players
function checkPassedQuestions() {
    
    console.log('Inside checkPassedQuestions');
    ...
    //showPassedQuestions()
    let userAnswer = answer.value.toLowerCase();
...
// This function will control the index of the main block of questions. After all questions are finished for both players the function will call checkPassedQuestions() to see if there are any questions passed and asked those passed questions. The last step, after all main and passed questions are asked, is to show the ranking and show 'game over'.
function checkIndex() {
...
function checkIndexPassedQuestions() {
    
    console.log('inside checkIndexPassedQuestions');
    
    switch(true) {
        // If all passed questions are finished, call ranking() and end game.
        case(firstPlayer.iQPassed > firstPlayer.qPassed.length && secondPlayer.iQPassed > secondPlayer.qPassed.length):
            ranking(firstPlayer, secondPlayer);
            document.write('Game Over');
            break;
...
// Force the 'intro' function to be loaded on page load
//window.onload = mainGame(); showQuestions();
